...
then
   ...
   jq  --arg src ${TARGET} --arg tgt ${TARGET}/context -r '.[] |if ( .urlref | startswith("/doc/applicationprofile") ) then  @sh "cp \($src)\(.seealso)/context/* \($tgt)" else empty end'  $LINKS | bash -e
   jq  --arg src ${TARGET} --arg tgt ${TARGET}/shacl -r '.[] | if ( .urlref | startswith("/doc/applicationprofile") ) then @sh "cp \($src)\(.seealso)/shacl/* \($tgt)" else empty end'  $LINKS | bash -e
